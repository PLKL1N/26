...
# ---- AWS Load Balancer Controller (ALB Ingress 처리) ----
...
# ---- Loki (Single Binary) ----
helm upgrade --install o11y-loki grafana/loki -n monitoring -f loki-values.yaml

# ---- OTel Collector (DaemonSet) ----
helm upgrade --install o11y-otel open-telemetry/opentelemetry-collector -n monitoring -f otel-values.yaml

# ---- App (ECR 이미지로 치환 후 배포) ----
sed "s#__ECR_URL__#${ECR_URL}#g" 10-app.yaml | kubectl apply -f -
...
echo
echo "=== Endpoints (ALB 프로비저닝까지 1~2분) ==="
echo "App ALB    : kubectl get ingress -n o11y log-generator"
echo "Grafana ALB: kubectl get ingress -n monitoring o11y-grafana"
